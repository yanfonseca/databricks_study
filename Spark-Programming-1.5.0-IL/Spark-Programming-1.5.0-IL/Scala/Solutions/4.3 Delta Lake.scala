...
val updatedSalesDF = salesDF.withColumn("items", size($"items"))
...
spark.sql("CREATE TABLE sales_delta USING DELTA LOCATION '%s'".format(deltaSalesPath))
...
// ANSWER
val oldSalesDF = spark.read.format("delta").option("versionAsOf", 0).load(deltaSalesPath)
display(oldSalesDF)
