...
val deltaDestFile = workingDir + "/people"
...
// We'll repartition before writing to ensure the part files are evenly balanced
...
// Now, instead of writing to Parquet, we can save the results directly to Delta. We'll also display the Delta files to make sure they were written as expected.
...
display( dbutils.fs.ls(deltaDestFile) )
