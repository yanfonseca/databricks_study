...
from pyspark.sql.functions import col, lag, lead, row_number, asc
...
(interpolatedDF
 .withColumn('row_number', row_number().over(dteWindow))
 .orderBy(asc('heartrate'))
).display()
...
interpolatedDF.where(col("heartrate") < 0).display()
